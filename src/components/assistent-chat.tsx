...
export function AssistentChat({
  ...
  return (
    <div className="flex min-h-[75vh] flex-col">
      <div className="kb-card flex-1 space-y-5 overflow-y-auto p-6">
        {berichten.length === 0 && (
        ...
        {berichten.map((b, i) => (
          <div
            key={b.id ?? i}
            className={b.role === 'user' ? 'ml-auto max-w-[640px]' : 'max-w-[720px]'}
          >
            {b.role === 'user' ? (
              <div className="rounded-lg bg-navy px-5 py-3 text-[15px] text-white">{b.content}</div>
            ) : (
              <div
