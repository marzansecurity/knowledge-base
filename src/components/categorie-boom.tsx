  ...
  return (
    <li>
      <Link
        ...
        className={`flex items-center justify-between rounded-md py-1.5 pr-2 text-[13px] transition-colors ${
          actief ? 'bg-navy text-white' : 'text-ink-soft hover:bg-page'
        }`}
      >
        <span>{categorie.name}</span>
        {aantal > 0 && (
          <span className={`text-[11px] ${actief ? 'text-white/70' : 'text-muted'}`}>{aantal}</span>
        )}
      </Link>
