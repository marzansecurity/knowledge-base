...
function LoginFormulier() {
  ...
  // De proxy bewaart dit pad zonder taalprefix; die zetten we er hier weer voor.
  ...
  async function verstuur(event: React.FormEvent) {
    ...
    if (aal?.nextLevel === 'aal2' && aal.currentLevel !== aal.nextLevel) {
      router.replace(
        `${pad(taal, '/login/verificeren')}?volgende=${encodeURIComponent(volgende)}`,
      );
      return;
    }
  ...
  return (
    <form onSubmit={verstuur} className="kb-card w-full max-w-sm p-7">
      ...
      <h1 className="text-[17px] font-bold text-navy">{t.login.titel}</h1>
      <p className="mt-0.5 text-[11px] text-muted">{t.login.ondertitel}</p>
      ...
      <button type="submit" disabled={bezig} className="kb-btn kb-btn-primary mt-6 w-full py-2">
        {bezig ? t.login.bezig : t.login.inloggen}
      </button>

      <p className="mt-5 text-[11px] leading-relaxed text-muted">{t.login.geenAccount}</p>
    </form>
  );
}
