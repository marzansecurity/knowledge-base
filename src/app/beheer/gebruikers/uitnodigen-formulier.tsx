...
import { nodigGebruikerUit } from './acties';
...
export function UitnodigenFormulier() {
  ...
  function verstuur(formData: FormData) {
    ...
    startTransitie(async () => {
      const resultaat = await nodigGebruikerUit(formData);
      if (resultaat.fout) {
        setMelding({ tekst: resultaat.fout, fout: true });
      } else {
        setMelding({ tekst: resultaat.succes ?? 'Uitnodiging verstuurd.', fout: false });
  ...
  return (
    <div className="kb-card p-5">
      <div className="kb-section-title mb-3">Nieuwe medewerker uitnodigen</div>
